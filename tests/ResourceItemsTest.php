...
final class ResourceItemsTest extends TestCase
{
    public function test_it_pages_through_resources_with_filters(): void
    {
        ...
        $page = $this->xenforo()->resourceItems()->list(1, ['type' => 'local', 'creator_id' => 3]);
    ...
    public function test_versions_come_back_as_a_plain_list(): void
    {
        ...
        $versions = $this->xenforo()->resourceItems()->versions(5);
    ...
    public function test_updates_and_reviews_are_paginated(): void
    {
        ...
        $this->assertTrue($this->xenforo()->resourceItems()->updates(5)->hasMore());
        ...
        $this->assertCount(1, $this->xenforo()->resourceItems()->reviews(5));
    ...
    public function test_it_creates_a_local_resource_against_an_attachment_key(): void
    {
        ...
        $resource = $this->xenforo()->resourceItems()->create([
    ...
    public function test_featuring_a_resource_can_carry_an_image(): void
    {
        ...
        $this->xenforo()->resourceItems()->feature(
            5,
            ['always_visible' => true],
    ...
    /**
     * The accessor is resourceItems(), not resources(), because resource() next to it is
     * the extension point and the two would read as the same thing. There is no assertion
     * here that resources() does not exist: PHPStan settles that at level 8, and a
     * method_exists() on a known class is dead code it can prove.
     */
    public function test_the_accessor_is_the_extension_point_under_a_name_of_its_own(): void
    {
        $xf = $this->xenforo();

        $this->assertInstanceOf(ResourceItems::class, $xf->resourceItems());
        $this->assertSame($xf->resourceItems(), $xf->resource(ResourceItems::class));
    }
}
