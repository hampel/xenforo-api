 ...
 *
 * WHY THE CLASS IS NOT CALLED `Resources`. Every other class here is named after its tag,
 * and this one is the exception on purpose: `Resource` is already the base class of all of
 * them, and `Client::resource()` is already this package's extension point. A `Resources`
 * beside those would make one word mean two unrelated things in the same namespace - and
 * `$xf->resources()` would read as "the resources this client has" rather than "XFRM's
 * resource items". XenForo's own entity is `XFRM\Entity\ResourceItem`, so this borrows that.
 *
 ...
 */
final class ResourceItems extends Resource
{
    /**
